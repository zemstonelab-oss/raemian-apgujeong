...
interface Props {
  data: ProjectData["sections"]["hero"];
}
...
export default function HeroSection({ data }: Props) {
  ...
  const totalPercDuration = masks.reduce((sum, m) => sum + m.duration, 0);
  ...
  return (
    <section className="relative h-screen overflow-hidden">
      ...
      <AnimatePresence>
        {masks.map((mask, idx) =>
          idx <= currentMask && idx < totalMasks ? (
            idx === currentMask ? (
              <motion.div
                key={idx}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-center gap-[0.4rem]">
              <span className="kb text-[1.3rem]">{data.subtitle.left}</span>
              <span
                className="h-[1px] bg-black transition-all duration-800"
                style={{ width: lineOn ? "24.5rem" : "0" }}
              />
              <span className="kb text-[1.3rem]">{data.subtitle.right}</span>
            </div>
            <div
